 ...
 *           
 * This is the application client in our solution where we connect to the server via socket to make different requests, like getting highest and lowest temp from database, calculate the average value, show all post and add post & delete post
 * (as for now, we send full MySQL-query directly from the application wich means
 * we dont have any error handeling of the datainput to the database, we did it this way due to the timeframe. Ideally we wouldent like to showcase any MySQL-code/queries in the application).
 * We have also edited some "port-forwarding" within our routersettings to make the communication work over internet.
 */
        ...
        private static void ConnectToServer()
        {
            ...
            while (!ClientSocket.Connected)
            {
                try
                {
                    ...
                    ClientSocket.Connect(IPAddress.Parse("111.111.1.1"), Port); //Enter the servers public IP-adress
                }
